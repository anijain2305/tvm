...
input_infos = {
    'test' : InputInfo('input', (1, 256, 16, 16), 'float32'),
...
class CompileDispatcher(object):
    ...
    def parse(self):
        ...
        if "test" in self.model_name:
            dtype = "int8"
            out_dtype = "int32"
            data = relay.var("data", shape=(1, 128, 16, 16), dtype=dtype)
            weight = relay.var("weight", shape=(16, 128, 3, 3), dtype=dtype)
            conv = relay.nn.conv2d(data, weight, out_dtype=out_dtype)
            mod = relay.Module.from_expr(conv)
            params = None 
        elif 'tflite' in self.model_name:
...
target = 'llvm -device=arm_cpu -target=armv7l-linux-gnueabihf -mattr=+neon'
# target = 'llvm -device=arm_cpu -target=armv7l-linux-gnueabihf -mattr=+neon'
# target = "llvm -device=arm_cpu -target=armv7l-linux-gnueabihf -mattr=+neon -mfloat-abi=soft"


# For Echo Gen 2
# target = "llvm -device=arm_cpu -target=arm-linux-android -mattr=+neon,+vfp4 -mfloat-abi=soft"
