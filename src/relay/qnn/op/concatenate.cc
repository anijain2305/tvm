 ...
 */
Expr ConcatenateLegalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  for (auto quantized_expr : tuple_data->fields) {
    ...
    if (input_scale != output_scale || input_zero_point != output_zero_point) {
      ...
      // Create the requantize attrs for calling the requantize op.
      auto requantize_attrs = make_node<RequantizeAttrs>();
      requantize_attrs->input_scale = input_scale;
      ...
      auto requantized_expr =
          RequantizeLower(quantized_expr, requantize_attrs.operator->(), input_shape, input_dtype);
