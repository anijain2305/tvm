...
/*
 * \brief Legalizes the QNN concatenate op.
 ...
 */
Expr QnnAddLegalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  if (lhs_scale != output_scale || lhs_zero_point != output_zero_point) {
    auto requantize_attrs = make_node<RequantizeAttrs>();
    requantize_attrs->input_scale = lhs_scale;
    requantize_attrs->input_zero_point = lhs_zero_point;
    ...
    requantized_lhs = RequantizeLower(lhs, requantize_attrs.operator->(), input_shape, input_dtype);
  }
  ...
  if (rhs_scale != output_scale || rhs_zero_point != output_zero_point) {
    auto requantize_attrs = make_node<RequantizeAttrs>();
    requantize_attrs->input_scale = rhs_scale;
    requantize_attrs->input_zero_point = rhs_zero_point;
    requantize_attrs->output_scale = output_scale;
    requantize_attrs->output_zero_point = output_zero_point;
    requantize_attrs->rounding = "TONEAREST";
    requantize_attrs->out_dtype = input_dtype;
    requantized_rhs = RequantizeLower(rhs, requantize_attrs.operator->(), input_shape, input_dtype);
  }
