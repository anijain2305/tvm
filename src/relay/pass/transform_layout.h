 ...
 *       - Use ctx transformation to get a Call with new layouts - GetCallWithNewLayouts.
...
Expr LayoutRewriter(const Call& ref_call, const Array<Expr>& new_args, const NodeRef& ctx) {
  ...
  T* ctx_transformer = static_cast<T*>(memorizer.operator->());
  ...
  // new_op = alter(op)
  Call new_call = ctx_transformer->GetCallWithNewLayouts(ref_call, normal_new_args);
