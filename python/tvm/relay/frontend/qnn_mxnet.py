...
def _mx_contrib_quantize(inputs, attrs):
    out_dtype = 'int8'
...
def _mx__sg_mkldnn_fully_connected(inputs, attrs, subgraphs, params):
    ...
    if len(_infer_type(data).checked_type.shape) == 4:
...
def _mx_mkldnn_conv(inputs, attrs, subgraphs, params):
    def _has_op(op_name):
        has_op = list()
        find_op = lambda x : \
                has_op.append(isinstance(x, tvm.relay.expr.Call) and x.op.name == op_name)
        relay.analysis.post_order_visit(subgraphs[0], find_op)
        return any(has_op)

    def _get_op_attrs(op_name):
        op_attrs = list()
        find_call_attrs = lambda x : \
                op_attrs.append(x.attrs) \
                if isinstance(x, tvm.relay.expr.Call) and x.op.name == op_name \
                else None
        relay.analysis.post_order_visit(subgraphs[0], find_call_attrs)
        assert len(op_attrs) == 1
        return op_attrs[0]

    def _get_quantized_data(data_index, data_min_index, data_max_index):
        # The firs index is quantized data tensor.
        data = inputs[data_index] 
        
        # Get the input data min/max range. Last 2 are min/max
        data_min = inputs[data_min_index]
        data_max = inputs[data_max_index]

        data_dtype = get_dtype_from_min_max(data_min, data_max)
        data_scale = get_mkldnn_uint8_scale(data_min, data_max) if data_dtype == 'uint8' \
            else get_mkldnn_int8_scale(data_min, data_max)
        data_zero_point = 0
        return (data, data_scale, data_zero_point)

    def _get_bn_alpha_coeff(bn_gamma_index, bn_beta_index, bn_running_mean_index,
            bn_running_var_index):
        bn_scale = None
        bn_shift = None
        if _has_op('nn.batch_norm'):
            # Extract relevant attrs from bn.
            bn_attrs = _get_op_attrs('nn.batch_norm')
            bn_epsilon_param = bn_attrs['epsilon']
            bn_scale_param = bn_attrs['scale']
            bn_center_param = bn_attrs['center']

            # Extract the relevant relay exprs.
            kernel = inputs[kernel_index]
            bn_running_var = inputs[bn_running_var_index]
            bn_gamma = inputs[bn_gamma_index]
            bn_beta = inputs[bn_beta_index]
            bn_running_mean = inputs[bn_running_mean_index]

            # Get coeff to multiply to weights.
            bn_epsilon =  relay.const(bn_epsilon_param, "float32")
            denominator = relay.sqrt(relay.add(bn_running_var, bn_epsilon))
            bn_scale = relay.divide(relay.const(1.0, "float32"), denominator)
            if bn_scale_param:
                bn_scale = relay.multiply(bn_gamma, bn_scale)
    ...
    def _get_quantized_kernel(kernel_index, bn_scale):
        # BN is folded into conv at runtime in MKLDNN. This folding is done at runtime.
        kernel = inputs[kernel_index]
        if _has_op('nn.batch_norm'):
            assert attrs.get_bool('with_bn', False)
            assert bn_scale is not None
            # Weights are on OIHW, and bn_scale is in O.
            bn_scale = relay.expand_dims(bn_scale, axis=1, num_newaxis=3)
            kernel = relay.multiply(bn_scale, kernel)
        ...
        # print("AAAAAAAAAAAAAAAAAAAAAAAAAAA", kernel_min, kernel_max)
        return quantize_conv_weights_mkldnn_from_var(kernel, kernel_min, kernel_max)

    def _get_kernel(kernel_index, bn_scale):
        # BN is folded into conv at runtime in MKLDNN. This folding is done at runtime.
        kernel = inputs[kernel_index]
        if _has_op('nn.batch_norm'):
            assert attrs.get_bool('with_bn', False)
    ...
    def _get_quantized_bias(bias_index, bn_scale, bn_shift, scale, zero_point):
        bias = None
        bias_scale = np.float32(data_scale * kernel_scale)
        bias_zero_point = zero_point
        if _has_op('nn.bias_add'):
            bias = inputs[bias_index]
            if _has_op('nn.batch_norm'):
                assert bn_shift is not None
                assert bn_scale is not None
                bias = relay.add(relay.multiply(bn_scale, bias), bn_shift)
        elif _has_op('nn.batch_norm'):
            assert bn_shift is not None
            assert bn_scale is not None
            bias = bn_shift
        return bias, bias_scale, bias_zero_point

    def _get_bias(bias_index, bn_scale, bn_shift):
        bias = None
        if _has_op('nn.bias_add'):
            bias = inputs[bias_index]
            if _has_op('nn.batch_norm'):
    ...
    def _get_qnn_conv2d(data, kernel, data_zero_point, kernel_zero_point):
        assert _has_op('nn.conv2d')
        
        # Extract relevant attrs from conv.
        conv2d_attrs = _get_op_attrs('nn.conv2d')

        # Create the QNN conv2d op.
    ...
    def _get_bias_add(res, bias, bias_scale, bias_zero_point):
        # if bias is not None:
        if _has_op('nn.bias_add'):
            assert bias_zero_point == 0

            # Extract relevant attrs from bias_add.
            bias_add_attrs = _get_op_attrs('nn.bias_add')

            int32_bias = quantize_conv_bias_mkldnn_from_var(bias, bias_scale)
            res = _op.nn.bias_add(res, int32_bias, axis=bias_add_attrs['axis'])
            # res = _op.nn.bias_add(res, int32_bias, axis=3)
        elif bias is not None:
            int32_bias = quantize_conv_bias_mkldnn_from_var(bias, bias_scale)
            res = _op.nn.bias_add(res, int32_bias, axis=1)
        return res

    def _get_bias_add_fp32(res, bias):
        # if bias is not None:
        if _has_op('nn.bias_add'):
            # Extract relevant attrs from bias_add.
            bias_add_attrs = _get_op_attrs('nn.bias_add')
            res = _op.nn.bias_add(res, bias, axis=bias_add_attrs['axis'])
            # res = _op.nn.bias_add(res, bias, axis=3)
        elif bias is not None:
            res = _op.nn.bias_add(res, bias, axis=1)
        return res


    def _get_requantized_op(res, data_scale, kernel_scale):
    ...
    def _get_sum(res, data_sum_index, data_sum_min_index, data_sum_max_index, data_scale, kernel_scale):
        # res is int32 quantized tensor with scale = data_scale * weight_scale, zp = 0
        # We can upcast the second tensor to int32 and get its scale and zero point
        # for the output sale and zero points we can look at calib ranges

        # Upcast the second input to int32
        data_sum = inputs[data_sum_index]
        data_sum_min = inputs[data_sum_min_index]
        data_sum_max = inputs[data_sum_max_index]

        data_sum_dtype = get_dtype_from_min_max(data_sum_min, data_sum_max)
        ...
        data_sum_zero_point = 0

        # Get ouptut scale, out_dtype. Output_zero_point = 0
        input_scale = np.float32(data_scale * kernel_scale)
    ...
    def _get_activation(res):
        if attrs.get_bool('with_act', False):
            assert attrs.get_bool('with_postsum_act', False) or attrs.get_bool('with_act', False)
            assert _has_op('nn.relu'), "Only Relu activation is supported"
            res = _op.nn.relu(res)
        return res

    if attrs.get_bool('quantized', False):
        # Find the indexes of different inputs.
        has_bias_add = _has_op("nn.bias_add")
        has_bn = _has_op("nn.batch_norm")
        bias_index = None 
        bn_gamma_index = None
        bn_beta_index = None
        bn_running_mean_index = None
        bn_running_var_index = None
        ...
        if has_bn:
            bn_gamma_index = bn_start_index
            bn_beta_index = bn_start_index + 1
            bn_running_mean_index = bn_start_index + 2
            bn_running_var_index = bn_start_index + 3

        data_sum = data_sum_min_index = data_sum_max_index = None
        if attrs.get_bool('with_sum', False):
            data_min_index = -4
            data_max_index = -3
            data_sum_index = -5
            data_sum_min_index = -2
            data_sum_max_index = -1
        else:
            data_min_index = -2
            data_max_index = -1
        
        # Extract data with quantized info.
        data, data_scale, data_zero_point = _get_quantized_data(data_index,
                                                                data_min_index,
                                                                data_max_index)

        # Extract bn_scale/shift. These will be used for folding BN into conv.
        bn_scale, bn_shift = _get_bn_alpha_coeff(bn_gamma_index,
                                                 bn_beta_index,
                                                 bn_running_mean_index,
                                                 bn_running_var_index)

        # Extract kernel with quantized info.
        kernel, kernel_scale, kernel_zero_point = _get_quantized_kernel(kernel_index,
                                                                        bn_scale)

        # print("BBBBBBBBBBBBB", kernel_scale, kernel_zero_point)
        # AAAAAAAAAAAAAAAAAAAAAAAAAAA -1.3226045 1.4842305
        # BBBBBBBBBBBBB 0.011686854499945432 0
        # Extract bias
        bias, bias_scale, bias_zero_point = _get_quantized_bias(bias_index,
                                                                bn_scale,
                                                                bn_shift,
                                                                data_scale * kernel_scale,
                                                                0)
        ...
        # Get Qnn conv2d
        res = _get_qnn_conv2d(data, kernel, data_zero_point, kernel_zero_point)

 
        # Get bias_add if applicable
        res = _get_bias_add(res, bias, bias_scale, bias_zero_point)

    
        if attrs.get_bool('with_sum', False):
            # Get the qnn add.
            res = _get_sum(res, data_sum_index, data_sum_min_index, data_sum_max_index,
                           data_scale, kernel_scale)
        else:
            # Requantize the ouptut
            res = _get_requantized_op(res, data_scale, kernel_scale)

        # Get the activation if needed.
        res = _get_activation(res)
        ...
        # Extract kernel
        kernel = _get_kernel(kernel_index, bn_scale)

        bias = _get_bias(bias_index,
                         bn_scale,
                         bn_shift)
        
        # Get Qnn conv2d
        res = _get_conv2d(inputs[0], kernel)

 
        # Get bias_add if applicable
        res = _get_bias_add_fp32(res, bias)
...
def _contrib_quantized_flatten(inputs, attrs):
    data = inputs[0]
    output_min = inputs[1]
    output_max = inputs[2]
...
def _contrib_dequantize(inputs, attrs):
    data = inputs[0]
    input_min = inputs[1]
    input_max = inputs[2]
...
def _contrib_quantized_act(inputs, attrs):
    act_type = attrs.get_str("act_type")
    assert len(inputs) == 3
...
_convert_map = {
    ...
    # Quantize
    "_contrib_quantize_v2" : _mx_contrib_quantize,
    "_sg_mkldnn_conv" : _mx_mkldnn_conv,
    "_contrib_quantized_flatten" : _contrib_quantized_flatten,
    ...
    for nid, node in enumerate(jnodes):
        ...
        if op_name == "null":
            ...
            if node_name in arg_params:
                dtype = np.dtype(arg_params[node_name].dtype).name
...
def _from_mxnet_impl(symbol, shape_dict, dtype_info, mod=None, arg_params=None):
    ...
    outputs = _from_mxnet_nodes(jgraph, shape_dict, dtype_info, mod, arg_params)
    outputs = outputs[0] if len(outputs) == 1 else _expr.Tuple(outputs)
...
def from_qnn_mxnet(symbol,
    ...
    if isinstance(symbol, mx.sym.Symbol):
        ...
        func = _from_mxnet_impl(symbol, shape, dtype, mod, params)
