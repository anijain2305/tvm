...
def _mx_mkldnn_conv(inputs, attrs, subgraphs, params):
    ...
    int8_kernel, kernel_scale, kernel_zero_point = \
        quantize_conv_weights_mkldnn_from_var(fp32_kernel, kernel_min, kernel_max)
    if len(inputs) == 4:
        conv_attrs = subgraph.attrs
    if len(inputs) == 5:
        conv_attrs = subgraph.args[0].attrs
    ...
    if len(inputs) == 5:
        ...
        int32_bias = quantize_conv_bias_mkldnn_from_var(fp32_bias, bias_scale)
        res = _op.nn.bias_add(res, int32_bias, axis=bias_attrs['axis'])
...
def from_qnn_mxnet(symbol,
    ...
    if isinstance(symbol, mx.sym.Symbol):
        ...
        func = _from_mxnet_impl(symbol, shape, dtype, mod, arg_params)
